...
class AudioCacheService {
  ...
  private async init(): Promise<void> {
    ...
    try {
      ...
      console.warn("⚠️ Failed to initialize audio cache:", error);
      throw error;
    }
  }
  ...
  private async cachePhrase(
    ...
    try {
      ...
      console.warn(`⚠️ Failed to cache phrase ${phraseId}:`, error);
      throw error;
    }
  }
  ...
  async getCachedAudio(phraseId: string): Promise<string | null> {
    try {
      ...
      console.warn(`⚠️ Failed to get cached audio for ${phraseId}:`, error);
      return null;
    }
  }
  ...
  async getCachedAudioByText(text: string): Promise<string | null> {
    try {
      ...
      console.warn("⚠️ Failed to get cached audio by text:", error);
      return null;
    }
  }
