...
export const ProfileScreen: React.FC = () => {
  ...
  const subscriptionConfigQuery = useQuery({
    ...
    enabled: !!subscriptionQuery.data?.stripe?.enabled,
  });
  ...
  const handleUpgrade = () => {
    if (!subscriptionQuery.data?.stripe?.enabled) {
      Alert.alert(
  ...
  const handlePlanSelection = async (
    ...
    if (!subscriptionQuery.data?.stripe?.enabled) {
      Alert.alert("Payments unavailable", "Billing is disabled in this build.");
      return;
    }
    if (plan.tier === "free") {
      Alert.alert(
