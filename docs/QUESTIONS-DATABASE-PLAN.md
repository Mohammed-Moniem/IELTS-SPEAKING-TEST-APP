...
export const IELTSQuestionModel = model<IIELTSQuestion>('IELTSQuestion', IELTSQuestionSchema);
```
...
export class IELTSQuestionController {
  ...
  @Get('/random')
  async getRandomQuestion(@QueryParam('category') category, @QueryParam('difficulty') difficulty) {
