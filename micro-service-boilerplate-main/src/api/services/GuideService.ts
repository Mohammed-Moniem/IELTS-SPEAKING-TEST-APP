...
const PUBLIC_GUIDE_CACHE = new Map<string, PublicCacheEntry<unknown>>();
const PUBLIC_GUIDE_CACHE_IN_FLIGHT = new Map<string, Promise<unknown>>();
...
export class GuideService {
  ...
  private readonly publicCacheEnabled = this.resolvePublicCacheEnabled();
  private readonly publicCacheTtlMs = this.resolvePublicCacheTtlMs();
  private readonly publicCacheMaxEntries = this.resolvePublicCacheMaxEntries();
  ...
  public async listPublicGuideTree(query: {
    ...
    const cacheKey = this.createPublicCacheKey('tree', {
      module: query.module || null,
      contentClass: query.contentClass || null,
      limit,
      offset,
      readMode: env.db.readMode,
      includeDrafts: this.shouldExposeDraftGuides()
    });
  ...
  public async getPublicGuidePageByPath(path: string) {
    ...
    const cacheKey = this.createPublicCacheKey('page', {
      normalizedPath,
      slugCandidate,
      readMode: env.db.readMode,
      includeDrafts: this.shouldExposeDraftGuides()
    ...
    return this.withPublicCache(cacheKey, async () => {
      const row = await this.fetchPublicGuidePageRow({
        normalizedPath,
        slugCandidate
      });
  ...
  public async listRelatedPublicGuides(guideId: string, limit = 6) {
    const normalizedLimit = this.resolvePagination(limit, 0, 6, 0, 30).limit;
    const cacheKey = this.createPublicCacheKey('related', {
      guideId,
    ...
    return this.withPublicCache(cacheKey, async () => {
      ...
      return {
        related: await this.findRelatedGuides(guideId, row.module, normalizedLimit)
      };
    });
  }
  ...
  public async searchPublicGuides(query: {
    ...
    const cacheKey = this.createPublicCacheKey('search', {
      q,
      module: query.module || null,
  ...
  public async importGuideSitemaps(input: { sitemaps?: string[]; inventoryDate?: string }, actorUserId: string) {
    ...
    try {
      ...
      await job.save();
      this.clearPublicCache('importGuideSitemaps:completed');
  ...
  public async generateGuideOutline(pageId: string, input: { strategyHint?: string }, actorUserId: string) {
    ...
    await page.save();
    this.clearPublicCache('generateGuideOutline');
  ...
  public async updateGuidePage(pageId: string, input: Record<string, unknown>, actorUserId: string) {
    ...
    await page.save();
    this.clearPublicCache('updateGuidePage');
  ...
  public async reviewGuidePage(
    ...
    await page.save();
    this.clearPublicCache('reviewGuidePage');
  ...
  public async publishGuidePage(pageId: string, actorUserId: string) {
    ...
    await page.save();
    this.clearPublicCache('publishGuidePage');
  ...
  public async enqueueGuideRefreshQueue(input: { module?: GuideModule; limit?: number }, actorUserId: string) {
    ...
    if (ids.length) {
      ...
      this.clearPublicCache('enqueueGuideRefreshQueue');
    }
  ...
  private normalizeGuideSummarySqlRow(row: Record<string, any>) {
    const toDate = (value: unknown, fallback?: string) => {
      const raw = typeof value === 'string' && value ? value : fallback;
      if (!raw) return undefined;
      const parsed = new Date(raw);
    ...
    return {
      _id: row.id,
      slug: row.slug || '',
      canonicalPath: normalizeGuideCanonicalPath(row.canonicalPath || '/ielts'),
      title: row.title || '',
      excerpt: row.excerpt || undefined,
      module: row.module as GuideModule,
      pageType: row.pageType as GuidePageType,
      contentClass: row.contentClass as GuideContentClass,
      templateType: row.templateType as GuideTemplateType,
      depth: Number.isFinite(Number(row.depth)) ? Number(row.depth) : 1,
      order: Number.isFinite(Number(row.order)) ? Number(row.order) : 100,
      parentId: row.parentId || undefined,
      state: row.state as GuideState,
      qaPassed: Boolean(row.qaPassed),
      qaScore: Number.isFinite(Number(row.qaScore)) ? Number(row.qaScore) : 0,
      publishedAt: toDate(row.publishedAt),
      lastReviewedAt: toDate(row.lastReviewedAt),
      updatedAt: toDate(row.updatedAt, row.rowUpdatedAt),
      createdAt: toDate(row.createdAt, row.rowCreatedAt)
    };
  }
  ...
  private async fetchPublicGuideRows(input: {
    ...
    if (!this.isSupabaseReadMode()) {
      ...
      const queryBuilder = GuidePageModel.find(filter).sort(sort).skip(input.offset).limit(input.limit);
    ...
    if (input.summaryOnly) {
      const summaryResult = await pool.query(
        `select
           id,
           data->>'slug' as slug,
           data->>'canonicalPath' as "canonicalPath",
           data->>'title' as title,
           data->>'excerpt' as excerpt,
           data->>'module' as module,
           data->>'pageType' as "pageType",
           data->>'contentClass' as "contentClass",
           data->>'templateType' as "templateType",
           data->>'parentId' as "parentId",
           data->>'state' as state,
           coalesce((data->>'qaPassed')::boolean, false) as "qaPassed",
           coalesce((data->>'qaScore')::float, 0) as "qaScore",
           coalesce((data->>'depth')::int, 1) as depth,
           coalesce((data->>'order')::int, 100) as "order",
           data->>'publishedAt' as "publishedAt",
           data->>'lastReviewedAt' as "lastReviewedAt",
           data->>'updatedAt' as "updatedAt",
           data->>'createdAt' as "createdAt",
           created_at as "rowCreatedAt",
           updated_at as "rowUpdatedAt"
         from "guide_pages" ${whereSql} ${orderSql} limit $${limitIdx}::int offset $${offsetIdx}::int`,
        params
      );

      return summaryResult.rows.map(row => this.normalizeGuideSummarySqlRow(row));
    }
    ...
    return detailResult.rows.map(row => this.normalizeGuideDocumentRow(row));
  }
  ...
  private resolvePagination(limit?: number, offset?: number, defaultLimit = 20, defaultOffset = 0, maxLimit = 100): Pagination {
    ...
    const parsedOffset = typeof offset === 'string' ? Number(offset) : Number(offset);
    const finalLimit = Number.isFinite(parsedLimit) ? Math.max(1, Math.min(maxLimit, parsedLimit)) : defaultLimit;
    const finalOffset = Number.isFinite(parsedOffset) ? Math.max(0, parsedOffset) : defaultOffset;
  ...
  private resolvePublicCacheEnabled(): boolean {
    const value = (process.env.GUIDE_PUBLIC_CACHE_ENABLED || '').trim().toLowerCase();
    if (value === 'true') return true;
    if (value === 'false') return false;
    return true;
  }

  private resolvePublicCacheTtlMs(): number {
    const parsed = Number(process.env.GUIDE_PUBLIC_CACHE_TTL_SECONDS || 300);
    const seconds = Number.isFinite(parsed) ? Math.max(5, Math.min(3600, parsed)) : 300;
    return seconds * 1000;
  }

  private resolvePublicCacheMaxEntries(): number {
    const parsed = Number(process.env.GUIDE_PUBLIC_CACHE_MAX_ENTRIES || 600);
    return Number.isFinite(parsed) ? Math.max(50, Math.min(5000, parsed)) : 600;
  }

  private createPublicCacheKey(scope: string, payload: unknown): string {
    return `${scope}:${this.stableStringify(payload)}`;
  }
  ...
  private async withPublicCache<T>(key: string, fetcher: () => Promise<T>, options?: PublicCacheOptions): Promise<T> {
    if (!this.publicCacheEnabled) {
      return fetcher();
    }

    const now = Date.now();
    const cached = PUBLIC_GUIDE_CACHE.get(key) as PublicCacheEntry<T> | undefined;
    if (cached && cached.expiresAt > now) {
      return cached.value;
    }

    const inflight = PUBLIC_GUIDE_CACHE_IN_FLIGHT.get(key) as Promise<T> | undefined;
    if (inflight) {
      return inflight;
    }

    const ttlMs = options?.ttlMs && Number.isFinite(options.ttlMs) ? Math.max(1, options.ttlMs) : this.publicCacheTtlMs;

    const pending = fetcher()
      .then(result => {
        PUBLIC_GUIDE_CACHE.set(key, {
          value: result,
          expiresAt: Date.now() + ttlMs
        });
        this.trimPublicCacheIfNeeded();
        return result;
      })
      .finally(() => {
        PUBLIC_GUIDE_CACHE_IN_FLIGHT.delete(key);
      });

    PUBLIC_GUIDE_CACHE_IN_FLIGHT.set(key, pending as Promise<unknown>);
    return pending;
  }

  private trimPublicCacheIfNeeded() {
    if (PUBLIC_GUIDE_CACHE.size <= this.publicCacheMaxEntries) {
      return;
    }

    const overflow = PUBLIC_GUIDE_CACHE.size - this.publicCacheMaxEntries;
    let removed = 0;
    for (const key of PUBLIC_GUIDE_CACHE.keys()) {
      PUBLIC_GUIDE_CACHE.delete(key);
      removed += 1;
      if (removed >= overflow) {
        break;
      }
  ...
  private clearPublicCache(reason: string) {
    if (!this.publicCacheEnabled) {
      return;
    }

    const size = PUBLIC_GUIDE_CACHE.size;
    PUBLIC_GUIDE_CACHE.clear();
    PUBLIC_GUIDE_CACHE_IN_FLIGHT.clear();
    this.log.info(`[GuideService] Cleared public guide cache (${size} entries) due to ${reason}`);
  }
