...
export function MarketingPageHero({
  ...
  return (
    <section
      className={`relative overflow-hidden rounded-2xl border border-violet-100 dark:border-violet-900/40 bg-gradient-to-r from-violet-900 via-violet-700 to-violet-600 text-white space-y-4 ${
        isCompact ? 'p-6' : 'p-8'
    ...
    >
      ...
      <h1 className={`relative z-10 font-bold ${isCompact ? 'text-2xl' : 'text-3xl'}`}>
        {title}
      </h1>

      {description ? (
        <p className="relative z-10 max-w-2xl text-white/80">{description}</p>
      ) : null}
