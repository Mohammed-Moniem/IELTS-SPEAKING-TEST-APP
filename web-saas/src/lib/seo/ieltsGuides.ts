export type IeltsGuide = {
  ...
  cluster?: 'speaking' | 'writing' | 'reading' | 'listening' | 'vocabulary-collocations' | 'exam-strategy' | 'institutions-coaches';
  overview: string;
  keyPoints: string[];
  actionPlan: string[];
...
export function getGuideAnswerBlock(guide: IeltsGuide) {
  if (guide.answerBlock) {
    return guide.answerBlock;
  }

  const shortAnswer = `${guide.title} improves fastest when you combine timed practice, direct feedback loops, and weekly full-checkpoint reviews.`;
  const whenToUse =
    guide.intent === 'transactional'
      ? 'Use this when your exam date is near and you need realistic readiness checks.'
      : guide.intent === 'commercial'
        ? 'Use this when you are building a repeatable weekly plan and need measurable score progression.'
        : 'Use this when you want a targeted strategy for one module before scaling to full mocks.';
  ...
  return {
    ...
    sourceBullets: [
      {
        label: 'Official IELTS guidance and test format',
        sourceName: 'IELTS.org',
        sourceUrl: 'https://www.ielts.org/'
      },
      {
        label: 'English level reference by CEFR',
        sourceName: 'Council of Europe CEFR',
...
export function getGuideIntentLinks(guide: IeltsGuide) {
  if (guide.intentLinks && guide.intentLinks.length > 0) {
    return guide.intentLinks;
  }
  ...
  const inferredModule =
    guide.cluster ||
    (guide.slug.includes('speaking')
      ? 'speaking'
      : guide.slug.includes('writing')
        ? 'writing'
        : guide.slug.includes('reading')
          ? 'reading'
          : guide.slug.includes('listening')
            ? 'listening'
            : 'exam-strategy');
  ...
  const links = [
    ...
    {
      href: '/pricing',
      label: `See ${guide.recommendedPlan} Plan`,
      description: 'Compare plans mapped to this strategy.'
    }
