...
test.describe.serial('real backend smoke', () => {
  test('registers, navigates learner routes, and validates core app/billing endpoints', async ({ page, request, baseURL }) => {
    ...
    const learnerRouteChecks: Array<{ path: string; heading: RegExp }> = [
      ...
      { path: '/app/speaking', heading: /practice, simulation, and quick evaluator/i },
      { path: '/app/progress', heading: /track ielts improvements/i },
      { path: '/app/billing', heading: /stripe checkout and package management/i },
      { path: '/app/settings', heading: /profile and preparation preferences/i }
    ];
