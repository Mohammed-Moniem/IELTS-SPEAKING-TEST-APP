...
test.describe('Full exam orchestration', () => {
  test('guides learner through complete full exam without manual IDs', async ({ page }) => {
    ...
    await expect(page.getByText('Active section: speaking')).toBeVisible();
    ...
    await expect(page.getByText('Active section: writing')).toBeVisible();
    ...
    await expect(page.getByText('Active section: reading')).toBeVisible();
    ...
    await expect(page.getByText('Active section: listening')).toBeVisible();
  ...
  test('resumes an interrupted exam from local recovery state', async ({ page }) => {
    ...
    await page.getByRole('button', { name: 'Start Reading Section' }).click();
    await expect(page.getByText('Reading Resume Section')).toBeVisible();
  });
});
